...
function [x, y, xx, yy, L1, L2, F, R, t] = generate_the_right_way(N)
    if nargin == 0
        N = 10;
    end
    ...
    points_in_space = rand(3, N);
    [L1, x, y] = space_to_image(points_in_space, f1, N);
    for i = 1 : N
        points_in_space(:, i) = R \ (points_in_space(:, i) - t); %actually inv(R) * (...), only faster and more accurate
    end
    [L2, xx, yy] = space_to_image(points_in_space, f2, N);
end